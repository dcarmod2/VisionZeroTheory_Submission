...
class tradeoff:
    ...
    def curve(self):
        points=pandas.DataFrame(numpy.nan,index=self.alphaList,columns=["rel_time","rel_APVM"])
        for n,alpha in enumerate(self.alphaList):
            ...
            apvm_mean=apvm_.sum()/multiplicity.sum()
            points.at[alpha,["rel_time","rel_APVM"]]=(time_mean,apvm_mean)
