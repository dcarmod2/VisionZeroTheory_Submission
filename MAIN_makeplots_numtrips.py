...
class tradeoff:
    ...
    def curve(self):
        points=pandas.DataFrame(numpy.nan,index=self.alphaList,columns=["rel_time","rel_trips"])
        for n,alpha in enumerate(self.alphaList):
            ...
            flags= ~ (numpy.isnan(frame["trip_time"]) | numpy.isnan(frame["trip_num_trips"]))
            ...
            num_trips_=frame["trip_num_trips"].multiply(multiplicity)
            time_mean=time_.sum()/multiplicity.sum()
            trips_mean=num_trips_.sum()/multiplicity.sum()
            points.at[alpha,["rel_time","rel_trips"]]=(time_mean,trips_mean)
    ...
    def avoidancePoints(self):
        ...
        return avoidance_frame.loc[:,["trip_time","trip_num_trips"]]
