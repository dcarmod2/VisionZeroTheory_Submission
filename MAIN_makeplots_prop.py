...
class tradeoff:
    ...
    def curve(self):
        points=pandas.DataFrame(numpy.nan,index=self.alphaList,columns=["alpha","time_by_APM"])
        for n,alpha in enumerate(self.alphaList):
            ...
            flags= ~ (numpy.isnan(frame["trip_time"]) | numpy.isnan(frame["trip_APM"]))
            ...
            points.at[alpha,["alpha","time_by_APM"]]=(alpha,(time_mean**1.5)*(apm_mean))
